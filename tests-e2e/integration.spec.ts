...
test('Bug creation and deletion flow', async ({ page }) => {
  ...
  const modalContent = await page.locator('[data-testid="add-bug-modal"]').innerHTML();
...
test('Adding a comment to a bug', async ({ page }) => {
  ...
  await page.fill('[data-testid="comment-content"]', `Test comment ${timestamp}`);
  await page.fill('[data-testid="comment-author"]', `Test User ${timestamp}`);
  await page.click('button:text("Add Comment")');

  await expect(page.locator(`p:text('Test comment ${timestamp}')`)).toBeVisible({ timeout: 10000 });
});
...
test('Editing a bug', async ({ page }) => {
  ...
  const formContent = await page.locator('[data-testid="edit-bug-form"]').innerHTML();
